...
const char *GeoIP_utf8_country_name[255] =
    ...
    "Benin", "Bermuda", "Brunei Darussalam", "Bolivia",
    ...
    "Togo", "Thailand",
    "Tajikistan", "Tokelau", "Turkmenistan", "Tunisia", "Tonga", "Timor-Leste",
        ...
        *q = 0;
        ...
        if (GeoIP_custom_directory[len - 1] != '/') {
            snprintf(path, sizeof(char) * 1024 - 1, "%s/%s",
                     GeoIP_custom_directory, file_name);
        } else {
            snprintf(path, sizeof(char) * 1024 - 1, "%s%s",
                     GeoIP_custom_directory, file_name);
        }
        ...
        if (stat(gi->file_path, &buf) != -1) {
            ...
            if (buf.st_mtime != gi->mtime
                ...
                if (gi->flags & GEOIP_MMAP_CACHE) {
...
#else
                    gi->cache =
                        mmap(NULL, buf.st_size, PROT_READ, MAP_PRIVATE,
                    ...
                    if (pread
                        (fileno(gi->GeoIPDatabase), gi->cache, buf.st_size,
                 ...
                 */
                idx_size =
                    _database_has_content(gi->databaseType) ?
                    gi->databaseSegments[0] * (long)gi->record_length *
                    2 : buf.st_size;
                                      ...
                                      GeoIPLookup * gl)
{
    ...
    for (depth = 127; depth >= 0; depth--) {
        if (gi->cache == NULL && gi->index_cache == NULL) {
            /* read from disk */
            silence =
                pread(fno, stack_buffer, gi->record_length * 2,
...
GeoIP *GeoIP_open(const char *filename, int flags)
{
    ...
    if (gi->GeoIPDatabase == NULL) {
        ...
        if (flags & GEOIP_INDEX_CACHE) {
            ...
            if (gi->index_cache != NULL) {
                if (pread
                    (fileno(gi->GeoIPDatabase), gi->index_cache, idx_size,
                     0) != idx_size) {
                    fprintf(stderr, "Error reading file %s\n", filename);
                    free(gi->databaseSegments);
                    free(gi->index_cache);
...
char *_get_name_gl(GeoIP * gi, unsigned long ipnum, GeoIPLookup * gl)
{
    ...
    if (gi->cache == NULL) {
        silence =
            pread(fileno(gi->GeoIPDatabase), buf, MAX_ORG_RECORD_LENGTH,
                  record_pointer);
